      ...
      <div className="print-area">
        {pages.map((page, index) => (
          <div key={page.id} className="relative">
            ...
            <div className={`document-page min-h-[1050px] p-4 border border-gray-300 shadow-lg bg-white mb-6 relative ${index > 0 ? 'page-break' : ''}`}>
              {/* Fixed Header */}
              <div 
                className="fixed-header pb-2 border-b border-gray-200 mb-3"
                dangerouslySetInnerHTML={{ __html: processedHeader }}
              />
              
              {/* Page Content */}
              <div 
                className="document-content min-h-[920px] leading-relaxed flex flex-col justify-start"
                dangerouslySetInnerHTML={{ __html: page.content }}
              />
              
              {/* Fixed Footer */}
              <div 
                className="fixed-footer pt-2 border-t border-gray-200 mt-3 absolute bottom-4 left-4 right-4"
